# Antigravity Conversation Fix (macOS and Cross-Platform)
...
This tool reconstructs the conversation index directly from your local `~/.gemini/` storage and repairs Antigravity's internal SQLite database (`state.vscdb`), restoring your conversations sorted by date with proper workspace mappings.
...
- **macOS and Linux Support**: Process lifecycle detection and path resolution (`~/Library/Application Support/Antigravity IDE`).
...
- **Multi-Database and Multi-Folder Merge**: Automatically merges and deduplicates conversations across `antigravity`, `antigravity-ide`, and backup directories.
- **Safe and Reversible**: Saves an automatic timestamped backup (`trajectorySummaries_backup_*.txt`) before applying any modifications.
...
- [Technical Architecture and Deep Dive](docs/technical_deep_dive.md)
